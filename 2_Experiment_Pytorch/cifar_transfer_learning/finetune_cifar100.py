        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if group['fixed_beta']!=0:
                    ...
                    momentum=1.0-torch.clamp(0.1*torch.sqrt(group['lr']/1e-4),
                                             min=0.05,
                                             max=0.95)
                ...
                v_f=torch.div(torch.acos(torch.tanh(torch.mul(m,grad))),math.pi)
                ...
                , min=0.81873075307,max=2.8010658347))
                ...
                vmax.copy_(torch.add(torch.mul(temp1,torch.add(1.0,-temp4)),torch.mul(temp4,pen)))
                s.copy_(pen)
        return loss
...
def main():
    ...
    for epoch in tqdm(range(start_epoch + 1, args.total_epoch)):
        ...
        np.savetxt('ft_100_%s_%d.txt'%(args.optim,args.size),np.array(log))
