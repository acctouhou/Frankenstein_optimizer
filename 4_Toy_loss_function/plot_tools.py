...
def plot_contourf(cost_f, figsize=[10, 10], _show=True):
    ...
    contour=ax.contourf(X, Y, Z, cmap=plt.cm.get_cmap(plt.cm.coolwarm), levels=np.linspace(zs.min(), zs.max(), 20))
    fig.colorbar(contour, shrink=1.0, aspect=20)

    plt.scatter(cost_f.x_optimum, cost_f.y_optimum,s=250,marker="x", color="black")
    if _show:
        plt.show()
    return fig, ax
